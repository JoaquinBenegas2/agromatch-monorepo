...
const ranked = (females: Female[]) => females.filter((f): f is ProfiledFemale => f.profile !== null).sort((a, b) => b.profile.traits.ci - a.profile.traits.ci || a.visualId.localeCompare(b.visualId) || a.id.localeCompare(b.id));
...
export function classifyHerd(females: Female[], farm: Farm, goal: BreedingGoal): Classification[] {
  ...
  return animals.map((female, index) => {
    ...
    if (percentile(index, n) <= 5 && t.pl < -0.5 && t.scs > 3.2 && t.rfi > 50) { tier = 'CULL_ALERT'; reasons.push('Alerta de descarte: CI bajo, PL bajo, SCS alto y RFI alto'); }
...
export function classifyHerdClassic(females: Female[]): Classification[] {
  ...
  return animals.map((female, index) => {
    ...
    const isBeef = t.ci < 355 || t.scs > 3.15 || t.fat < 0 || t.pro < 0;
    const isElite = !isBeef && t.ci > 480 && t.pro > 0 && t.fat > 0 && t.scs < 3;
