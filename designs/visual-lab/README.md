...
**Referencia visual vigente del design system.** Esta es la dirección elegida (`index.html`, especialmente `#/mercado`): paisaje low poly, verde profundo + lima + arcilla sobre papel vegetal, formas en pastilla y acentos serif itálicos. `/designs/tokens.css` y `apps/frontend/src/styles.css` están alineados a esta paleta y forma — ver ambos como fuente de verdad de implementación; estas pantallas quedan como referencia de intención visual, no se tocan para construir producto.
