...
export function useMatchBoard(farmId: string, femaleId: string | undefined, goal: BreedingGoal) {
  return useQuery({
    queryKey: queryKeys.matches(farmId, femaleId ?? '', hashGoal(goal)),
    queryFn: () => api.post(`/farms/${farmId}/females/${femaleId}/matches`, { goal }, MatchBoardSchema),
    enabled: Boolean(femaleId),
  });
}
