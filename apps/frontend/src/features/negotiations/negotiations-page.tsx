...
export function NegotiationsPage() {
  ...
  if (negotiations.error)
    return <ErrorMessage message={negotiations.error.message} />;
    ...
    return (
      <div className="py-6">
        <EmptyState
          ...
          description={
            user.role === 'PROVIDER'
              ? 'Las consultas de productores aparecerán acá.'
              : 'Iniciá una desde un proveedor recomendado en el marketplace.'
          }
          action={
            user.role !== 'PROVIDER' ? (
  ...
  return (
    <div className="flex flex-col gap-4 py-5 sm:py-7">
      <PageHeader
        title="Negociaciones y mensajes"
        description="Conversá con productores y proveedores; el acuerdo se realiza por fuera de AgroMatch."
      />
      <div className="grid min-w-0 gap-4 xl:grid-cols-[290px_minmax(0,1fr)]">
        <Card
          className={cn('min-w-0 overflow-hidden', id && 'hidden xl:block')}
        >
          <CardHeader>
            <CardTitle>Conversaciones ({items.length})</CardTitle>
          </CardHeader>
          <div className="max-h-[65dvh] overflow-y-auto divide-y divide-border-soft">
            {items.map((item) => (
              <button
                type="button"
                key={item.id}
        ...
        <Card
          className={cn(
            'min-w-0 h-[70dvh] min-h-[320px] flex-col overflow-hidden',
            id ? 'flex' : 'hidden xl:flex',
          )}
        >
          <CardHeader>
            <Button
              variant="ghost"
              className="mb-2 w-fit xl:hidden"
              onClick={() => navigate('/negociacion/matches')}
            >
              ← Conversaciones
            </Button>
            <CardTitle>
          ...
          <ChatThread className="min-h-0">
            {selected.messages.map((item) => (
              <ChatBubble
                key={item.id}
                align={item.senderType === ownType ? 'end' : 'start'}
          ...
          {send.error ? (
            <div className="px-3">
              <ErrorMessage message={send.error.message} />
            </div>
          ) : null}
          {closed ? (
            <div className="border-t border-border p-4 text-center text-[11.5px] text-muted-foreground">
            ...
            <ChatComposer
              ...
              onSend={() =>
                send.mutate(
                  { id: selected.id, body: { body: message.trim() } },
        ...
        <Card className={cn('xl:col-start-2', !id && 'hidden xl:block')}>
          <CardContent className="flex gap-3 text-[11.5px]">
            <CalendarDays className="size-4 shrink-0 text-primary" />
            <div>
              <strong>{selected.farmName}</strong>
              <p className="text-muted-foreground">
                {selected.category} · conversación iniciada{' '}
                {formatDate(selected.createdAt)}. El acuerdo se coordina por
                fuera de AgroMatch.
              </p>
            </div>
          </CardContent>
