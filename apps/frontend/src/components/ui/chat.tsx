...
function ChatBubble({ className, align = 'start', author, timestamp, children, ...props }: ChatBubbleProps) {
  const isEnd = align === 'end';
  return (
    <div
      data-slot="chat-bubble"
      className={cn('flex flex-col gap-1', isEnd ? 'items-end' : 'items-start', className)}
      {...props}
    >
      <div
        className={cn(
          'max-w-[70%] rounded-lg px-3 py-2 text-[13px] leading-relaxed',
          isEnd ? 'bg-primary text-primary-foreground' : 'bg-muted text-foreground',
        )}
      >
        {children}
      </div>
...
export interface ChatComposerProps extends Omit<React.ComponentProps<'form'>, 'onSubmit'> {
  value: string;
