...
import type { BreedingGoal, BreedingPlan, PlanItem } from '@org/shared-types';
import { BreedingGoalSchema, PlanItemSchema } from '@org/shared-types';
...
export class PlanController {
  ...
  addItem(
    ...
    @Body(new ZodValidationPipe(PlanItemSchema)) item: PlanItem,
  ...
  @Get('export.csv')
  async exportCsv(@Param('farmId') farmId: string, @Res() res: Response): Promise<void> {
    ...
    const femalesById = new Map<string, FemaleCsvInfo>(
      females.map((f) => {
        const tier = classificationRecord?.items.find((c) => c.femaleId === f.id)?.tier ?? '';
    ...
    res.setHeader('Content-Disposition', `attachment; filename="plan-${farmId}.csv"`);
    res.send(csv);
  }
}
