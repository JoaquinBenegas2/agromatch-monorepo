...
import type {
  CreateReviewBody, CreateServiceRequestBody, Review, ServiceRequest, User,
...
import { SERVICE_REQUEST_REPO, type ServiceRequestRepo } from '../../repos/service-request.port.js';
...
export class RequestsService {
  ...
  async create(needId: string, body: CreateServiceRequestBody, user: User): Promise<ServiceRequest> {
    ...
    if (!provider) {
      throw new DomainError('PROVIDER_NOT_FOUND', 'El proveedor no existe', 404, {
        providerId: body.providerId,
      });
    }
    return this.requests.create({
      id: randomUUID(), needId, providerId: provider.id, message: body.message,
      status: 'SENT', createdAt: new Date().toISOString(), contact: provider.contact,
    });
  }

  async review(requestId: string, body: CreateReviewBody, user: User): Promise<Review> {
    const request = await this.requests.findById(requestId);
    if (!request) {
      throw new DomainError('REQUEST_NOT_FOUND', 'La solicitud no existe', 404, { requestId });
    }
    const need = await this.needs.findById(request.needId);
    if (!need) {
      throw new DomainError('NEED_NOT_FOUND', 'La necesidad no existe', 404, { needId: request.needId });
    }
    assertNeedAccess(user, need);
    ...
    const review = await this.reviews.create({
      id: randomUUID(), serviceRequestId: request.id, providerId: provider.id,
      rating: body.rating, comment: body.comment, createdAt: new Date().toISOString(),
    });
    const jobs = provider.reputation.jobs + 1;
    const previousTotal = (provider.reputation.avg ?? 0) * provider.reputation.jobs;
